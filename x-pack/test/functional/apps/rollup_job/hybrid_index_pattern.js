...
export default function ({ getService, getPageObjects }) {
  ...
  describe('hybrid index pattern', function () {
    ...
    it('create hybrid index pattern', async () => {
      ...
      const indexPatternNames = (await PageObjects.settings.getIndexPatternList()).map(
        async (i) => {
          const name = await i.getVisibleText();
          if (name.includes(rollupIndexPatternName) && name.includes('Rollup')) {
            return name;
          }
        }
      );
      expect(indexPatternNames.length).to.be(1);
    });
